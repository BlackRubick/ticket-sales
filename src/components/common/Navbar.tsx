...
export const Navbar: React.FC = () => {
  ...
  const navigation = [
    ...
    },
    {
      ...
      href: '/scanner',
      icon: '📱',
      description: 'Validar boletos',
      permission: 'canViewScanner' as const
    },
    {
      name: 'Boletos',
      href: '/tickets',
      ...
      permission: 'canViewTickets' as const
    },
  ].filter(item => permissions[item.permission]); // Solo mostrar items con permiso
          ...
          <div className="flex justify-between h-16">
            ...
            <div className="flex items-center">
              ...
              <div className="flex-shrink-0 flex items-center">
                <a href={navigation.length > 0 ? navigation[0].href : "/sales"} className="flex items-center space-x-2 group">
              ...
              <div className="relative">
                ...
                {showUserMenu && (
                  <div className="absolute right-0 mt-2 w-64 rounded-2xl shadow-xl bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50 transform opacity-100 scale-100 transition-all duration-200">
                    <div className="p-4 border-b border-gray-100">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className={`text-xs font-medium px-2 py-1 rounded-full ${getRoleColor(user?.role || '')}`}>
                            {getRoleDisplayName(user?.role || '')}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="py-2">
                      {permissions.canViewTicketResend && (
                        <a
                          href="/tickets/resend"
          ...
          <div className="border-t border-gray-200 pt-6">
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div>
                <p className="font-medium text-gray-900">{user?.name}</p>
                <p className="text-sm text-gray-500">{user?.email}</p>
                <p className={`text-xs font-medium px-2 py-1 rounded-full inline-block mt-1 ${getRoleColor(user?.role || '')}`}>
                  {getRoleDisplayName(user?.role || '')}
                </p>
              </div>
            </div>
